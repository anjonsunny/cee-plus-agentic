...
- **Note (deferred, has a trust ripple):** surfacing `fluid_encoded_as_state` as a `check_graph_rule_conformance` rule in `main.py` (+ `RULE_TO_FAMILY` + `CONSEQUENCE_CATEGORY` + the 7-step consistency check) fires on ~15 scenes and shifts β/trust/batch numbers, so it is a deliberate second step pending Sunny's go, not part of this self-contained intervention.py fix.
